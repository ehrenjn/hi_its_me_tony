...
from consts import (
    ...
    BATCH_SIZE, NEURONS_PER_HIDDEN_LAYER, NULL_WORD
)
...
class TimeSeriesMessage(TimeSeriesInput):
    ...
    def make_output(self):
        ...
        for index in range(len(output)):
            if index < len(self._output_message.words):
                word = self._output_message.words[index]
                vect = self._vectorizer.to_vect(word)
                output[index] = vect
            else:
